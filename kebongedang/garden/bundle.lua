...
return function(ctx)
	...
	pcall(function()
		...
		local RECON = ("if getgenv then getgenv().GAG_BRANCH=%q end;loadstring(game:HttpGet('https://raw.githubusercontent.com/catursec/testing/%s/kebongedang/init.lua'))()"):format(branch, branch)
...
local function sendWebhook(url, payload, ctx)
	...
	if type(payload) == "table" then
		...
		if not payload.avatar_url then payload.avatar_url = "https://raw.githubusercontent.com/catursec/testing/main/kebongedang/Logo/logo_icon.png" end
	end
...
local AVATAR = "https://raw.githubusercontent.com/catursec/testing/main/kebongedang/Logo/logo_icon.png"
		...
		embeds = {
			{
				...
				color = 3066993,
				description = desc,
				footer = { text = os.date("%B %d | %I:%M %p"), icon_url = "https://raw.githubusercontent.com/catursec/testing/main/kebongedang/Logo/logo_icon.png" },
			}
		}
	}
end
		...
		embeds = {
			{
				...
				footer = {
					...
					icon_url = "https://raw.githubusercontent.com/catursec/testing/main/kebongedang/Logo/logo_icon.png"
				}
			}
		}
	}
	...
	local function handleEggMinus(eggName, amt, thresh)
		...
		if ctx.reconnect then
			...
			if q then
				local branch = (getgenv and getgenv().GAG_BRANCH) or _G.GAG_BRANCH or "main"
				pcall(function()
					q(('if getgenv then getgenv().GAG_BRANCH=%q end loadstring(game:HttpGet("https://raw.githubusercontent.com/catursec/testing/%s/kebongedang/init.lua"))()'):format(branch, branch))
				end)
			end
			task.wait(0.4)
			local TeleportService = game:GetService("TeleportService")
...
return function(ctx)
	...
	local function queueLoader()
		local q = (syn and syn.queue_on_teleport) or queue_on_teleport
			or (fluxus and fluxus.queue_on_teleport) or (getgenv and getgenv().queue_on_teleport)
		if q then
			local cmd = ('if getgenv then getgenv().GAG_BRANCH=%q end loadstring(game:HttpGet("https://raw.githubusercontent.com/catursec/testing/%s/kebongedang/init.lua"))()'):format(branch, branch)
			pcall(function() q(cmd) end)
		end
	end
